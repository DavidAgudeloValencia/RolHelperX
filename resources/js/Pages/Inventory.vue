...
function editModal(inventory, id) {
    form.tittle = inventory.tittle
    form.description = inventory.description
    form.amount = inventory.amount
    form.id = id
    isOpen.value = true
}
...
const submitCreate = () => {
    if (form.id == 0) {
        form.post(route("inventory"), {
            onSuccess: () => isOpen.value = false
        });
    } else {
        form.patch(route("inventory"), {
            onSuccess: () => isOpen.value = false
        });
    }
};
                        ...
                        <DisclosurePanel class="px-4 pt-4 pb-2 text-sm text-gray-500">
                            ...
                            <div class="mt-4 flex justify-between">
                                <button type="button"
                                    ...
                                    @click="editModal(inventory, inventory.id)">
                                    Edit
                                </button>
                                <button type="button"
                            ...
                            <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                                ...
                                <DialogPanel
                                    ...
                                    <div class="mt-2 flex items-center justify-evenly">
                                        <InputLabel for="amount" value="Amount" class="text-white" />
                                        <TextInput id="amount" type="text" class="mt-1 block w-1/2"
                                            oninput="this.value = this.value.replace(/[^0-9]/g, ''" v-model="form.amount"
